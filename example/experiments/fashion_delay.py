...
train_data = torchvision.datasets.FashionMNIST('f_mnist_data', train = True, download = True,  transform=sample_transform, target_transform=label_transform)
test_data = torchvision.datasets.FashionMNIST('f_mnist_data', train = False, download = True, transform=sample_transform, target_transform=label_transform)
...
class DNetwork(torch.nn.Module):
    def __init__(self, netParams, init_stats = None):
        ...
        if init_stats is None: 
            self.dfc1 = slayer.vectorizedDelayedDense(Nin, Nhid, delay_init_lower = hyper_params['delay_init_lower'], delay_init_upper = hyper_params['delay_init_upper'])
            ...
            self.dfc3 = slayer.vectorizedDelayedDense(256, Nout, delay_init_lower = hyper_params['delay_init_lower'], delay_init_upper = hyper_params['delay_init_upper'])
        else:
            self.dfc1 = slayer.vectorizedDelayedDense(Nin, 800, init_stats=init_stats[0], delay_init_lower = hyper_params['delay_init_lower'], delay_init_upper = hyper_params['delay_init_upper'])
            self.dfc3 = slayer.vectorizedDelayedDense(800, Nout, init_stats=init_stats[1], delay_init_lower = hyper_params['delay_init_lower'], delay_init_upper = hyper_params['delay_init_upper'])
